...
unmarshalReference' pckt pos p_len = with Monad do
  ...
  case res of
    ...
    Just (lbls ## (Right (_ ## ref)), res_len) => do
      ...
      pure $ [| (Just res) ++ rest |]
      --pure $ Just (pack $ map (chr . val) xs) (doesn't work for some reason)
...
parseDNSQuestion (encoded_domain ## qt ## qc) = do
  ...
  case decoded_domain of
    ...
    Right decoded_domain' =>
      pure $ Right (MkDNSQuestion decoded_domain' qt qc)
...
decodeDomainPayload dom_pl = do
  ...
  case domain of
    Left err => pure $ Left err
    Right domain' => pure $ Right (DNSDomainPayload domain')
...
decodeSOAPayload (mn ## rn ## ser ## ref ## ret ## exp ## min) = do
  ...
  case (mn', rn') of
    (Right mn'', Right rn'') =>
      pure $ Right (DNSSOAPayload (MkSOA mn'' rn'' (val ser) (val ref)
...
decodePayload DNSPayloadRelIP ip_pl = pure $ Right (DNSIPv4Payload (decodeIP ip_pl))
...
decodePayload _ _ = pure $ Left (PayloadUnimplemented)
      ...
      case (domain, getPayloadRel pl_ty ty cls) of
        ...
        (Right domain', Right pl_rel) => do
          ...
          case decoded_pl of
            Left err => pure $ Left err
            Right decoded_pl' =>
              pure $ Right (MkDNSRecord domain' ty cls (val ttl) pl_rel decoded_pl')
...
encodeSOA soa = do
  ...
  pure $ (mname ## rname ## serial ## refresh ## retry ## expire ## minimum)
...
encodeDNS (MkDNS hdr qc ac nsc arc qs as auths ars) = with Monad do
    ...
    pure (encoded_hdr ## b_qc ## b_ac ## b_nsc ## b_arc ## qs'' ## as'' ## auths'' ## ars'')
