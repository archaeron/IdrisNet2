...
getLocalPaddlePos = do
  st <- get
  if isServer st then
    pure $ pongLeftPaddlePos st
  else
    pure $ pongRightPaddlePos st
...
requiresLaunching = do
  ...
  pure (pongToLaunch st && (pongBallStuck ball))
...
updateBallServer = do
  ...
  pure $ requires_launching ||  paddle_collision ||  lr_bounds || tb_bounds
