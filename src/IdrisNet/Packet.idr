...
marshalChunk (ActivePacketRes pckt pos p_len) (Prop _) x2 = pure 0 -- We're not doing anything
...
unmarshalCString' (ActivePacketRes pckt pos p_len) i with (pos + 8 <= p_len)
  ...
  | True = do
    ...
    if (char == '\0') then do
      ...
      case rest of Just (xs, j) => pure $ Just (char::xs, j + 8)
...
unmarshalCString (ActivePacketRes pckt pos p_len) = do
  ...
  case res of
       Just (chrs, len) => pure $ Just (pack chrs, len)
       Nothing => pure Nothing

unmarshalLString' : ActivePacket -> Nat -> IO (List Char)
unmarshalLString' ap Z = pure []
...
unmarshalBits (ActivePacketRes pckt pos p_len) (Bit width p) with ((pos + (natToInt width) - 1) <= p_len)
  | True = do
    ...
    pure $ Just $ (BInt res (assert_total $ believe_me Oh), (natToInt width)) -- Have to trust it, as it's from C
  | False = pure Nothing
unmarshalBits _ _ = pure Nothing
...
unmarshalChunk (ActivePacketRes pckt pos p_len) (Decodable n t decode_fn _) = do
  ...
  if (end_pos <= p_len) then do
    ...
    case (decode_fn (BInt dat (believe_me Oh))) of -- Decode using supplied fn
      Just decoded_val => pure $ Just (decoded_val, len)
      Nothing => pure Nothing
  else pure Nothing -- Out of bounds
unmarshalChunk _ (Prop p) =
  case unmarshalProp p of
       Just p' => pure $ Just (p', 0)
       Nothing => pure Nothing
...
unmarshal' (ActivePacketRes pckt pos p_len) (c >>= k) = do
  ...
  pure ((res ** res2), res_len + res2_len)
...
--  dumpPacket pckt 1024
  pure $ (unmarshal' (ActivePacketRes pckt 0 (len * 8)) pl)
